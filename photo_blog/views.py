...
def search(request):
    ...
    if 'searchpost' in request.GET:
        ...
        if query:
            queryset = Post.objects.all().filter(
                Q(caption__icontains=query) | Q(geolocation__icontains=query)
            ).distinct()
            #print("search post2")
        context = {
...
#     if request.is_ajax():
#         data1 = request.POST.get('latitude')
#         data2 = request.POST.get('longitude')
#
#         print(data1)
#         print(data2)
#         geolocator = Nominatim()
#         location = geolocator.reverse((data1, data2))
#         print(location.address)
#         return HttpResponse("Get the location!")
